...
function isValidInput(name) {
   ...
   for ( i = 0; i < name.length; i++ ) {
      if ( isUnsafeInput(name.charAt(i)) == true )
         return false;
   }
...
function isValidDomain(val, spec_domain, igcase) {
	var list_id;
	var post=val.indexOf('@');
	var str=val.substring(post,val.length);
	var IsRegEx=spec_domain.indexOf('^');
	
	if(spec_domain.length==0)
		return true;
	//if spec_domain is not blank , which means need checking
	if(post<1)
		return false;
	
	if(IsRegEx<0)//means it's not Regular Expression
	{
	    domain_list = spec_domain.split('|');	
	    list_id = 0;
	    for(list_id=0;list_id<domain_list.length;list_id++)
	    {
	        if(str==domain_list[list_id])
	        return true;
	    }
	    return false;
	}
	else//use RegExp to check the domain name.
	{
	    var myReg;
	    if(igcase)
	        myReg = new RegExp(spec_domain.replace(/\./g,"\\\."), "i");
	    else
	        myReg = new RegExp(spec_domain.replace(/\./g,"\\\."));
	    if (myReg.test(str)) return true;
	    return false;
	}
}

function isUnsafeInput(compareChar) {
   var unsafeString = "\"";
   var unsafeString1 = "\'";
   var unsafeString2 = "\\";
   var unsafeString3 = "\<";
   ...
   if ( unsafeString4.indexOf(compareChar) == -1 && unsafeString3.indexOf(compareChar) == -1 && unsafeString2.indexOf(compareChar) == -1 && unsafeString1.indexOf(compareChar) == -1 && unsafeString.indexOf(compareChar) == -1 && compareChar.charCodeAt(0) > 32
        && compareChar.charCodeAt(0) < 123 )
      return false; // found no unsafe chars, return false
   else
      return true;
...
function check_goodwords(str, goodwords) {
 	var this_str;
...
function isUnsafeURL(compareChar) { //only for urlfilter
   
   if (( unsafeURL.indexOf(compareChar) == -1 ) && compareChar.charCodeAt(0) > 32
        && compareChar.charCodeAt(0) < 123 )
      return false; 
   else
      return true;
...
function isValidURL(name) {
   var i = 0;	
   for ( i = 0; i < name.length; i++ ) {
      if ( isUnsafeURL(name.charAt(i)) == true )
         return false;
   }
   return true;
}